...
extract_words <- function(script) {
 ...
 # funs <- str_extract_all(string = script_, pattern = "[[:alnum:]\\._]+|<.*?>(.*?)</.*?>")
   funs <- str_extract_all(string = script_, pattern = "ins|del")
...
#'
get_script_funs <- function(script, filter_context = TRUE
                             , highlight = TRUE
                            ) {
     packages_funs <- get_package_funs(script)
  ...
  script_funs$valid_context <- TRUE
  if (filter_context) {
